...
- $t_{\mathrm{SW}}$ — Per-round CPU dispatch budget on each device: $t_{\mathrm{SW}} = L \cdot k \cdot \tau_{\mathrm{launch}} + 2 \cdot PP \cdot k_{\mathrm{pp\_hop}} \cdot \tau_{\mathrm{launch}}$. The second term counts inter-stage P2P launches ($\approx 2$ per microbatch × $PP$ microbatches per round); inert when $PP = 1$ (kernel_launch_overhead.md §5).
