...
- $t_{\text{prefill,local}}$: roofline local time, *with two corrections* added to the legacy $\max(t_{\text{compute}}, t_{\text{mem}})$ form:
  ...
  - SW composition with the per-stage CPU dispatch budget $t_{\mathrm{SW}}^{\mathrm{stage}} = (L/PP) \cdot k \cdot \tau_{\mathrm{launch}} + k_{\mathrm{pp\_hop}} \cdot \tau_{\mathrm{launch}}$ (the second term counts the recv + send P2P kernels at this stage's PP boundary, inert when $PP = 1$), applied via the SW-overlap factor $\rho_{\mathrm{SW}}$:
