...
**Parameter traffic (100% load per token)** 
$$
T_{\theta} \approx
\frac{L}{PP}
\left(
...
**Activation traffic (optimized by FA)** 
$$
T_{\text{act}} =
...
\left(
\frac{H^2 + 3HH_{kv}}{TP}
+ \frac{3HI N_{\text{exp}}}{TP\cdot EP}
+ c_{\text{act}}H
...
### EP all-to-all (2 pass: Dispatch + Combine)
Ring:
$$
t_{EP}^{\text{ring}} =
2(EP-1)\alpha_{EP}
+ 2(EP-1)\frac{kH b}{EP B_{\text{eff,EP}}}
$$
...
+ 2\frac{kH b}{B_{\text{eff,EP}}}
$$

### TP all-reduce (2 pass, Full Vector $H$)
Ring:
$$
t_{TP}^{\text{ring}} =
2(TP-1)\alpha_{TP}
+2\frac{TP-1}{TP}\frac{H b}{B_{\text{eff,TP}}}
$$
