...
| Scale-up Switch | $t_\mathrm{coll}(M, G) = \alpha_\mathrm{span}(G) + \beta \cdot M / \mathrm{BW_\mathrm{span}}(G)$; algorithm-dependent $\beta$: ring-AR $= 2(G-1)/G$, A2A $= (G-1)/G$, P2P $= 1$; $\alpha_\mathrm{span}$ and $\mathrm{BW_\mathrm{span}}$ come from flattening the fabric tier-chain, innermost first | [`switching.md`](documentation/modeling/switching.md) |
