...
**Hierarchical scale-up/out.** Each parallelism domain is described by an ordered list of switching tiers (innermost first), each with its own radix $P_i$, per-port bandwidth $\mathrm{BW}_i$, and latency $\alpha_i$. A collective over $G$ ranks crosses the minimum number of tiers needed to reach all ranks; multi-tier all-reduce decomposes as inner reduce-scatter → outer sub-AR → inner all-gather, with payload telescoping shrinking the cross-tier traffic. A single-tier list reproduces the legacy flat $(\alpha_{\mathrm{role}}, \mathrm{BW_\mathrm{role}})$ model exactly; multi-tier configurations (e.g. NVL72 intra-rack NVSwitch + inter-rack aggregation; d-Matrix `pair_mesh → pcie_server → ethernet_rack` for scale-out across servers) are supported via a `"tiers": [...]` JSON form. See [`modeling/collectives.md`](documentation/modeling/collectives.md) §3.3 "Hierarchical RS → sub-AR → AG composition" and the `pareto_vs_scale_up_tier` case study for a worked example.
