...
**Hierarchical scale-up/out.** Each parallelism domain is described by an ordered list of switching tiers (innermost first), each with its own radix $P_i$, per-port bandwidth $\mathrm{BW}_i$, and latency $\alpha_i$. A collective over $G$ ranks crosses the minimum number of tiers needed to reach all ranks; multi-tier all-reduce decomposes as inner reduce-scatter → outer sub-AR → inner all-gather, with payload telescoping shrinking the cross-tier traffic. A single-tier list reproduces the legacy flat $(\alpha_\mathrm{role}, \mathrm{BW}_\mathrm{role})$ model exactly; multi-tier configurations (e.g. NVL72 intra-rack NVSwitch + inter-rack aggregation; d-Matrix `pair_mesh → pcie_server → ethernet_rack` for scale-out across servers) are supported via a `"tiers": [...]` JSON form. See [`modeling/collectives.md`](documentation/modeling/collectives.md) §3.3 "Hierarchical RS → sub-AR → AG composition" and the `pareto_vs_scale_up_tier` case study for a worked example.
...
| Decode Cluster | $t_\mathrm{local} = \max(t_\mathrm{compute}, t_\mathrm{mem})$; $t_\mathrm{comm} = t_\mathrm{TP} + t_\mathrm{EP} + t_\mathrm{SP} + t_\mathrm{PP}$ (per-role collectives summed per pipeline stage, each priced by the Scale-up/out Network row below); $t_\mathrm{token} = t_\mathrm{local} + \max(0, t_\mathrm{comm} - \rho \cdot t_\mathrm{local})$; $\mathrm{TPOT} = t_\mathrm{token} / B$; memory→compute crossover at $B^* = T_\theta \cdot R / (F_\mathrm{token} \cdot \mathrm{BW_\mathrm{eff,0}} - T_\mathrm{kv} \cdot R)$ (single-tier closed form; see Memory Hierarchy row below for the multi-tier form) | [`decode.md`](documentation/modeling/decode.md) |
| Memory Hierarchy | $t_\mathrm{mem}(B) = \sum_i (T_{\theta,i} + B \cdot T_{\mathrm{KV},i}) / \mathrm{BW}_{\mathrm{eff},i}$ — multi-tier opening of the decode roofline (single-tier reduction recovers $T_\mathrm{step} / \mathrm{BW_\mathrm{HBM}}$ exactly) | [`sram.md`](documentation/modeling/sram.md) |
| Scale-up/out Network | $t_\mathrm{coll}(M, G) = n_\alpha \cdot \alpha \cdot \eta_\alpha + n_\beta \cdot M / (\mathrm{BW} \cdot \eta_\beta)$ per shipped primitive (Hockney α–β with contention factors $\eta_\alpha \ge 1$, $\eta_\beta \in (0, 1]$); ring/DBT for star AR; dim-decomposed for torus; hierarchical RS → sub-AR → AG with payload telescoping for multi-tier crossbars; INC variants on `sharp_class` / `hw_a2a` tiers short-circuit to switch-cut-through $\alpha$ + 1× BW; per-primitive $n_\alpha$, $n_\beta$ in collectives.md §3-§6 | [`collectives.md`](documentation/modeling/collectives.md) |
...
├── documentation/
│   ├── modeling/                     — methodology derivations (notation, decode, prefill, e2e, kv, framework, collectives, sram, dram3d)
...
└── llm_perf/
    ...
    │   ├── decode_model.py           — decode FLOPs, traffic, comm, multi-tier roofline + overlap-aware TPOT, B*
    ...
    │   └── primitives/               — phase-agnostic physics (weight/KV footprint, linear FLOPs, collective cost dispatch)
    ├── database/
    ...
    │   └── tuner/                    — tuner JSONs (collective algorithms, MemoryPlacementSpec, S_decode, B_decode, ...)
...
### `pareto_vs_scale_up_tier.ipynb` — decode Pareto under scale-up tiering + collective-algorithm choice

![decode Pareto: ideal NVL576 baseline vs rail-optimized 2-tier under 3 algorithm modes (ring, optimized SW, INC)](assets/pareto_vs_scale_up_tier.png)
...
**Setup:** 4 frontiers on the same 576 × GB200-class hardware. The "ideal" single-tier baseline is unchanged (576-port flat NVLink, hypothetical upper bound). The "rail-optimized 2-tier" replaces the original flat-IB outer with an 8-pod rail-leaf topology (NVLink5 + Quantum-SHARP IB; α_outer = 2 μs, BW_outer = 400 GB/s) — this is the realistic NVL576 SuperPOD shape. On the rail-optimized side, three algorithm modes are compared: **ring** (every collective pinned to ring; INC off), **optimized SW** (`tp/ep_algorithm_*='auto'` + `optimize_collective_algorithms`; INC off), and **INC** (auto + `inc_enabled=True`; NVLS on the inner tier, Quantum SHARP on the outer). `TP_CHOICES` and `EP_CHOICES` are derived from the model: `TP_max = n_kv = 16`, `EP_max = n_experts = 16` for GPT-1.8T MoE.

**Headline:** for this model (`n_kv=16`, `n_experts=16`), all four Pareto frontiers overlap exactly — 35,643 of 35,643 evaluation points are identical because every valid partition keeps TP, EP ≤ 16 and SP ≤ 64, so every collective stays inside one NVL72 rack (≤ 72 ranks) and never touches tier 1. Algorithm choice is invisible at the frontier level when no group spans the rack boundary. The 72-rank cliff is real — §6 shows a forced TP=144 stress test where ideal+ring pays 144 μs while rail+INC pays only 2.8 μs (the multi-tier scale-out INC formula `2k·α_switch + M/BW` with k=2 tiers collapses the α-term from `2(143)·0.5` to ~1.2 μs and halves the BW term via the AR-only `BW_eff` doubling). With INC available on both tiers, the cliff effectively disappears for AR/AG. EP A2A still hits the cliff because sharp_class doesn't accelerate A2A. **Extending scale-up past NVL72 buys nothing at decode unless the model's per-role parallelism routinely exceeds 72 ranks** (large-TP dense, fine-grained MoE with many hundreds of experts, or prefill/training collectives). See `documentation/modeling/collectives.md` §3.3 for the multi-tier hierarchical model and §3.4 for the INC scale-out formula.
